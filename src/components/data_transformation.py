...
class DataTransformation:
    ...
    def initialize_data_transformation(self, train_path, test_path):
        try:
            logging.info(f"Attempting to read train data from {train_path}")
            train_df = pd.read_csv(train_path)
            logging.info(f"Train data shape: {train_df.shape}")

            logging.info(f"Attempting to read test data from {test_path}")
            ...
            # Test dataset might not have the target column
            target_feature_test_df = test_df[target_column_name] if target_column_name in test_df else None
